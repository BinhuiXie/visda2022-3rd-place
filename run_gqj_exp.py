...
import torch
from mj_exps import generate_experiment_cfgs
...
if __name__ == '__main__':
    ...
    if args.config is not None:
        setproctitle.setproctitle(f'VisDA GQJ#CFG: {args.config}')
        ...
        child_cfg = {
            ...
            'name': unique_name,
            'work_dir': os.path.join('work_dirs_GQJ', exp_name, unique_name),
    ...
    if args.exp is not None:
        setproctitle.setproctitle(f'VisDA GQJ#EXP: {args.exp}')
        ...
        for i, cfg in enumerate(cfgs):
            ...
            cfg['name'] = f'{datetime.now().strftime("%y%m%d_%H%M")}_' \
                          f'{cfg["name"]}_{str(uuid.uuid4())[:5]}'
            cfg['work_dir'] = os.path.join('work_dirs_GQJ', exp_name, cfg['name'])
