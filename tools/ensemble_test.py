...
# Modifications: Support ensemble test
import argparse
import os
...
from mmseg.apis import multi_gpu_test, single_gpu_test, multi_gpu_test_ensemble, single_gpu_test_ensemble
from mmseg.datasets import build_dataloader, build_dataset
from mmseg.models import build_segmentor

from tools import EnsemblePolicy
...
def main():
    ...
    assert len(args.config) == len(args.checkpoint), 'unable to ensemble'

    cfgs = []
    for cfg_i in args.config:
        cfg = mmcv.Config.fromfile(cfg_i)
    ...
    # set cudnn_benchmark
    if cfgs[0].get('cudnn_benchmark', False):
    ...
    data_loader = build_dataloader(
        dataset,
        samples_per_gpu=1,
        workers_per_gpu=cfgs[0].data.workers_per_gpu,
        dist=distributed,
        shuffle=False)

    models = []
    for cfg, ckpt in zip(cfgs, args.checkpoint):
    ...
    if not distributed:
        models = [MMDataParallel(model, device_ids=cfg.gpu_ids) for model in models]
        results = single_gpu_test_ensemble(
            models,
            data_loader,
            args.show,
            args.show_dir,
